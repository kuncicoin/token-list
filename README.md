...
Kunci Token Registry is a package that allows application to query for list of tokens.
...
```bash
npm install @kunci/spl-token-registry
```

```bash
yarn add @kunci/spl-token-registry
```
...
import { TokenListProvider, TokenInfo } from '@kunci/spl-token-registry';
...
Submit PR with changes to JSON file `src/tokens/kunci.tokenlist.json`
